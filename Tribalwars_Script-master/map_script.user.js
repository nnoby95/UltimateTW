            ...
            'success': function (data) {
                ...
                if (outgoingTable) {
                    ...
                    if (outgoing_unitsElement.length > 0) {
                        ...
                        // Wait for map elements to load before adding icons
...
function addOutgoingIcons() {
    ...
    outgoingCommands.forEach(command => {
        ...
        icons.forEach(function (icon, index) {
            ...
            // Adds the icon before the village in the DOM
...
async function getReportsList() {
    ...
    // Create a map to store the most recent report by coordinate
...
function extractReports(tempElement) {
    ...
    // Function to compare dates and determine which is more recent
                    ...
                    if (report.attackLootResults || report.attackLootDiscoverResults) {
                        ...
                        $.ajax({
                            ...
                            'success': function (data) {
                                ...
                                // Create line with last attack date
                                var tr = document.createElement('tr');
                                tr.id = "info_last_attack";
                                var th = document.createElement('th');
                                th.innerHTML = '↓Last Attack:  ';
...
function addFakeFarmAssistantButton() {
    ...
    newButton.onclick = async function fetchData() {
        try {
            const response = await fetch(game_data.link_base_pure + "place&target=" + villageId, {
                headers: {
                    "tribalwars-ajax": "1",
    ...
    // Add the new button to ctxButtons
