...
function storeAvailableUnitsCosts(data) {
    ...
    $(data).find(".recruit_req").each(function(index, reqEl) {
        $(reqEl).find('span').each(function(i, span) {
            ...
            if (match) {
                let unitType = match[1]; // Unit name (spear, sword, etc.)
                let resource = match[2]; // Resource type (wood, stone, iron, pop)
    
                // Ensures the unit has an object before adding costs
