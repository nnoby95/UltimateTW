...
function createWidgetElement({ identifier, contents, columnToUse, update, extra_name = '', description = '', title = ''}) {
    ...
    if (columnElement) {
        var elemId = identifier.toLowerCase().replace(/ /g, '_'); // Gets the element title
        ...
        header.onclick = function () {
            ...
            if (miniElem.src.includes('minus')) {
                ...
                // Finds the corresponding object in the widgets array and updates the 'open' value to false
                settings_cookies.widgets.find(widget => widget.name === elemName).open = false;
            } else {
                miniElem.src = miniElem.src.replace('plus', 'minus');
                // Finds the corresponding object in the widgets array and updates the 'open' value to true
...
function extractBuildTimestampFromHTML(stringHTML) {
    ...
    if (stringToday && stringTomorrow) {
        ...
        if (day !== undefined && hora !== undefined && minuto !== undefined) {
            ...
            if (day === 1) { // If it's tomorrow, add 1 day
...
function autoDailyBonusCollect() {
    //need to get the day easily... I can do a fetch to create the first time and then count from there...
    //I just need to know what happens when you don't collect one day
    if (game_data) {
...
function sendScavengeAjax() {
    //need to get the troops to send... if there are numbers in settings those are the ones
    //if not, I have to get them
    // need to have the level to send
    if (game_data) {
        const squadRequest = {
            village_id: 14520,
            candidate_squad: {
